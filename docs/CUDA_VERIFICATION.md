...
```bash
docker run --rm --gpus all --privileged -e BACKEND=cuda harbor.lan/stability-toys:test \
...
1. **Check --gpus flag:**
   ```bash
   # Wrong (missing --gpus)
   docker run --rm harbor.lan/stability-toys:test python verify_cuda.py

   # Correct
   docker run --rm --gpus all harbor.lan/stability-toys:test python verify_cuda.py
   ```
...
```bash
docker build --platform linux/amd64 -f Dockerfile.test --build-arg BACKEND=cuda -t harbor.lan/stability-toys:test .
docker run --rm --gpus all -e BACKEND=cuda harbor.lan/stability-toys:test python verify_cuda.py
```
...
```bash
# Use GPU 0 only
docker run --rm --gpus '"device=0"' harbor.lan/stability-toys:test python verify_cuda.py

# Use GPU 1 only
docker run --rm --gpus '"device=1"' harbor.lan/stability-toys:test python verify_cuda.py

# Use multiple GPUs
docker run --rm --gpus '"device=0,1"' harbor.lan/stability-toys:test python verify_cuda.py
```
...
```bash
# Limit GPU memory
docker run --rm --gpus all --memory=16g harbor.lan/stability-toys:test python verify_cuda.py
```
