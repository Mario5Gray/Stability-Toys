...
3. **`worker` access across the boundary (finding C).** The Governor accesses `self._handle.worker` in several places (demand-reload check, mode-switch skip, `is_model_loaded`). `worker` is a **read-only `@property` on the `WorkerHandle` ABC** (`@property def worker(self) -> Optional[PipelineWorker]: ...`), so the coupling is interface-level, not private-attr-level: `InProcessWorkerHandle.worker` returns `self._worker`; `StubHandle.worker` returns `None`. This is still temporary coupling — facet-3's `SubprocessWorkerHandle` has no in-proc worker, so its `worker` property returns `None` and the Governor uses `handle.health().state` instead. Future cleanup replaces `self._handle.worker is None` with `self._handle.health().state == "dead"`.
