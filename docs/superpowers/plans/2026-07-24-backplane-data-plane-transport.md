...
3. **In-proc never reconstructs exceptions.** The compat Subscriber calls `fut.set_exception(err.original)` — the live instance. The `code → exception_factory` reconstruction table is exercised only by the IPC transport (Task 5) and only for `OOM` / `CANCELLED` / `GENERIC`. Reconstructing `StaleResolutionError` across a real IPC boundary is deferred (the real stale path is in-proc this issue), avoiding a `worker_pool ↔ backplane` import cycle.
