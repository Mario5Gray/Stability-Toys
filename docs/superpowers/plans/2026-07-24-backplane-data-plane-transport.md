...
- Facet-3: move CudaWorker to a spawn subprocess (consumes both transports).
  ...
  - **IPC transport hardening (from Task 5 review of `d60a52a`) — facet-3 prerequisites before a real IPC consumer:**
    - **`request(n)` backpressure.** `_IpcSubscription.request` is a no-op (`ipc.py`), so demand never crosses the pipe and the conflating operator (dormant in-proc) can't activate for IPC — a slow real consumer would be flooded. Wire `request(n)` as a reverse control frame (spec §6.2) and move conflation to the producer side.
    - **`IpcJobSink(conn, job_id)`.** `job_id` is currently hardcoded `"job"` on every emitted frame; thread the real `job_id` through construction so `job_id`-correlation (spec §4.3) holds for a real consumer.
    - **`result()` signature.** IPC `result()` coerces raw `bytes` and passes a pre-made non-`SharedMemBlob` straight to `encode_frame` (which would assert). Tighten to the bytes-path contract (IPC allocates the segment) or document it explicitly.
