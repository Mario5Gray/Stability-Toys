...
- Push the selected branch to `origin/<branch>` before touching the remote worktree
...
4. Push the branch to `origin/<branch>` using a non-force push.
5. SSH to `enigma`.
...
10. If `.worktrees/<branch>` already exists, refresh that checkout to exactly `<remote>/<branch>`.
...
- reset the worktree to `<remote>/<branch>`

This keeps repeated invocations cheap and predictable while ensuring testing uses the branch state that was just pushed.
...
`--dry-run` should print the resolved values and the commands that would be executed without mutating either host.
...
```text
enigma:~/workspace/Stability-Toys/.worktrees/gallery-ux-polish
```

That output is the handoff point for manual test commands on `enigma`.
