...
- [`server/upload_routes.py`](/Users/darkbit1001/workspace/Stability-Toys/server/upload_routes.py) routes `POST /v1/upload` to a `server/asset_store.py` bucket by the request's `type` — control-map types (`canny`/`depth`/`pose`) to the durable `control_map` bucket, `image`/`ref` to `ref_image`, and any other or missing type to the ephemeral `upload` bucket — and validates routed uploads as decodable images. It still does not own the reusable-map lifecycle: `server/asset_store.py` owns storage, budgets, and eviction, backing both upload refs and emitted control-map refs.
